...
namespace DriveHUD.PKCatcher.Licensing
{
    ...
    internal class LicenseService : ILicenseService
    {
        ...
        public void ResetLicenses()
        {
            ...
            var licenseAssembly = typeof(PKTRegistration.PKTReg).Assembly;
            ...
            foreach (var fileToDelete in filesToDelete)
            {
                ...
                {
                    LogProvider.Log.Error(CustomModulesNames.PKCatcher, $"Could not delete license file at '{fileToDelete}'", e);
                    errorOccurred = true;
                }
            }
            ...
            if (Directory.Exists(mirrorFolder))
            {
                try
                {
                    ...
                    LogProvider.Log.Info(CustomModulesNames.PKCatcher, $"Mirror folder has been deleted");
                }
                catch (Exception e)
                {
                    LogProvider.Log.Error(CustomModulesNames.PKCatcher, $"Mirror folder has not been deleted", e);
                }
            }
            ...
            throw new PKInternalException(new NonLocalizableString("Some license files haven't been deleted."));
        }
        ...
        private Dictionary<string, string> ParseException(Exception ex)
        {
            ...
            {
                LogProvider.Log.Error(CustomModulesNames.PKCatcher, "Could not parse exception", e);
            }
        ...
        private void UpdateExpirationDates(IEnumerable<ILicenseInfo> licenses)
        {
            ...
            {
                LogProvider.Log.Error(CustomModulesNames.PKCatcher, "Getting expiration date failed - collections do not match");
                return;
            }
        ...
        private DateTime[] GetExpiryDates(string[] serialHashes)
        {
            ...
            {
                LogProvider.Log.Error(CustomModulesNames.PKCatcher, "Could not obtain expiry dates", e);
        ...
        public IEnumerable<ILicenseInfo> LicenseInfos
        {
            get
            {
                if (!isInitialized)
                {
                    throw new PKInternalException(new NonLocalizableString("License has not been initialized"));
                }
        ...
        public bool IsUpgradable
        {
            get
            {
                return !licenseInfos.Any(x => x.IsRegistered && !x.IsExpired && x.Serial.StartsWith("PKP"));
            }
        }
        ...
        {
            if (serial.StartsWith("PKS", StringComparison.Ordinal)
                || serial.StartsWith("PKP", StringComparison.Ordinal))
            {
                return LicenseType.PKCNormal;
            }

            if (serial.StartsWith("PKT", StringComparison.Ordinal))
            {
                return LicenseType.PKCTrial;
            }
        ...
        private static IEnumerable<GameType> ConvertLicenseType(LicenseType licenseType)
        {
            ...
            switch (licenseType)
            {
                case LicenseType.PKCNormal:
                case LicenseType.PKCTrial:
                    ...
                    break;
                default:
                    throw new PKInternalException(new NonLocalizableString("Not supported license type"));
            }
