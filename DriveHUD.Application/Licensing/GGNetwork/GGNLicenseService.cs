...
namespace DriveHUD.Application.Licensing.GGNetwork
{
    ...
    /// License service
    /// </summary>
    internal class GGNLicenseService : IGGNLicenseService
    {
        ...
        private readonly List<IGGNLicenseInfo> licenseInfos;

        private bool isInitialized;

        public GGNLicenseService()
        {
            licenseInfos = new List<IGGNLicenseInfo>();
        }
        ...
        public bool Validate()
        {
            ...
            var licenseTypes = new GGNLicenseType[]
            {
                GGNLicenseType.GGNTrial,
                GGNLicenseType.GGNHoldem,
                GGNLicenseType.GGNOmaha,
                GGNLicenseType.GGNCombo
            };

            // validate each possible type 
            foreach (GGNLicenseType licenseType in licenseTypes)
            {
                ...
                var licenseInfo = new GGNLicenseInfo(license, licenseType);
        ...
        public bool Register(string serial, string email)
        {
            ...
            {
                licenseInfo = new GGNLicenseInfo(license, licenseType.Value);
                licenseInfos.Add(licenseInfo);
        ...
        private void UpdateExpirationDates(IEnumerable<IGGNLicenseInfo> licenses)
        {
        ...
        private DateTime[] GetExpiryDates(string[] serialHashes)
        {
            ...
            try
            {
                using (var server = new GGNDeployLXLicensingServer())
                {
        ...
        /// </summary>
        public IEnumerable<IGGNLicenseInfo> LicenseInfos
        {
            get
            {
                if (!isInitialized)
        ...
        public bool IsUpgradable
        {
            get
            {
                return !licenseInfos.Any(x => x.IsRegistered && !x.IsExpired && x.Serial.StartsWith("GGNC"));
            }
        }
        ...
        public GGNLicenseType? GetTypeFromSerial(string serial)
        {
            if (serial.StartsWith("GGNH", StringComparison.Ordinal))
            {
                return GGNLicenseType.GGNHoldem;
            }

            if (serial.StartsWith("GGNO", StringComparison.Ordinal))
            {
                return GGNLicenseType.GGNOmaha;
            }

            if (serial.StartsWith("GGNT", StringComparison.Ordinal))
            {
                return GGNLicenseType.GGNTrial;
            }

            if (serial.StartsWith("GGNC", StringComparison.Ordinal))
            {
                return GGNLicenseType.GGNCombo;
            }
        ...
        private static IEnumerable<GameType> ConvertLicenseType(GGNLicenseType licenseType)
        {
            var gameTypes = new List<GameType>();

            switch (licenseType)
            {
                case GGNLicenseType.GGNHoldem:
                ...
                case GGNLicenseType.GGNOmaha:
                    ...
                    break;
                case GGNLicenseType.GGNCombo:
                case GGNLicenseType.GGNTrial:
