//-----------------------------------------------------------------------
// <copyright file="BaseAppStoreRepository.cs" company="Ace Poker Solutions">
...
{
    internal abstract class BaseAppStoreRepository<T> : IBaseAppStoreRepository<T>
        where T : class
    {
        ...
        private IEnumerable<T> GetAppStoreProducts()
        {
            try
            {
                using (var stream = File.Open(storeLocalProductRepo, FileMode.Open, FileAccess.Read, FileShare.Read))
                {
                    var xmlSerializer = new XmlSerializer(typeof(AppStoreProductStorage<T>));
                    var appStoreProductStorage = xmlSerializer.Deserialize(stream) as AppStoreProductStorage<T>;
