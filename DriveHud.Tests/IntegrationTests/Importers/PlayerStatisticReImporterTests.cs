    ...
    class PlayerStatisticReImporterTests : BaseDatabaseTest
    {
        ...
        private void AssertThatIndicatorsAreEqual(HudLightIndicators actual, HudLightIndicators expected)
        {
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(actual.ThreeBetInBB, Is.EqualTo(expected.ThreeBetInBB), nameof(HudLightIndicators.ThreeBetInBB));
                Assert.That(actual.ThreeBetInBTN, Is.EqualTo(expected.ThreeBetInBTN), nameof(HudLightIndicators.ThreeBetInBTN));
                Assert.That(actual.ThreeBetInCO, Is.EqualTo(expected.ThreeBetInCO), nameof(HudLightIndicators.ThreeBetInCO));
                Assert.That(actual.ThreeBetInMP, Is.EqualTo(expected.ThreeBetInMP), nameof(HudLightIndicators.ThreeBetInMP));
                Assert.That(actual.ThreeBetInSB, Is.EqualTo(expected.ThreeBetInSB), nameof(HudLightIndicators.ThreeBetInSB));
                ...
                Assert.That(actual.ColdCallInBB, Is.EqualTo(expected.ColdCallInBB), nameof(HudLightIndicators.ColdCallInBB));
                Assert.That(actual.ColdCallInBTN, Is.EqualTo(expected.ColdCallInBTN), nameof(HudLightIndicators.ColdCallInBTN));
                Assert.That(actual.ColdCallInCO, Is.EqualTo(expected.ColdCallInCO), nameof(HudLightIndicators.ColdCallInCO));
                Assert.That(actual.ColdCallInMP, Is.EqualTo(expected.ColdCallInMP), nameof(HudLightIndicators.ColdCallInMP));
                Assert.That(actual.ColdCallInSB, Is.EqualTo(expected.ColdCallInSB), nameof(HudLightIndicators.ColdCallInSB));
                ...
                Assert.That(actual.MRatio, Is.EqualTo(expected.MRatio), nameof(HudLightIndicators.MRatio));
                Assert.That(actual.StackInBBs, Is.EqualTo(expected.StackInBBs), nameof(HudLightIndicators.StackInBBs));
